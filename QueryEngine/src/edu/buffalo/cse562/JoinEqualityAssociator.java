...
public class JoinEqualityAssociator implements ExpressionVisitor
{
	// CLASS VARIABLES
	...
	public JoinEqualityAssociator(Column[] schema) 
	{
		this.schema = schema;
	...
	public void visit(EqualsTo expression) 
	{
		...
		{
			if (!leftData.equals("notinschema") && !rightData.equals("notinschema"))
			{
				indexList.add(leftCol.getTable().getName() + "." + leftIndex + "=" + rightCol.getTable().getName() + "." + rightIndex);
			}
	...
	public void visit(Column expression) 
	{
		...
		for (int i = 0 ; i < schema.length ; i++)
		{
			...
			{
				if (expression.toString().equalsIgnoreCase(schema[i].getTable().getAlias()+"."+schema[i].getColumnName()) 
						|| expression.toString().equalsIgnoreCase(schema[i].getTable().getName()+"."+schema[i].getColumnName()))
				{
					index = i;
					break;
				}
